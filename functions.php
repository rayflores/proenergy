...
/**
 * Remove page editor on post_type = page
 */
add_action( 'init', 'remove_wp_editor_pages' );
		...
		$('a[data-name=add-layout]').click(function(){
			waitForEl('.acf-tooltip li', function() {
				$('.acf-tooltip li a').hover(function(){
					...
					$('.acf-tooltip').append('<div class="imagePreview"><img src="<?php echo $siteURL; ?>/wp-content/themes/proenergy/modules/editor/' + imageTP + '.png"></div>');
				}, function(){
					$('.imagePreview').remove();
				});
				});
